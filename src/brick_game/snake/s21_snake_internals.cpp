 ...
 * - движение и рост змейки
 * - генерацию яблок
 * - проверку столкновений с границами и телом
 * - обновление счёта, уровня и скорости
 * - загрузку и сохранение рекорда
 ...
 *   доступ только через `create()` / `destroy()` / `handle_input()` и др.
 * - **Жизненный цикл**: управление памятью осуществляется строго через
 * статические методы.
 * - **Модульная структура**: методы разбиты по функциональным блокам:
 *   - Инициализация: `initialize_()`, `initializeSnake_()`
 *   - Ввод и FSM: `handle_input()`, `mapActionToEvent_()`, `processEvent_()`
 *   - Логика движения: `move_()`, `checkCollision_()`, `eatApple_()`
 *   - Генерация: `spawnApple_()`
 *   - Обновление данных: `updateFieldState_()`, `updateOccupiedCells_()`
 *   - Работа с сохранениями: `loadHighScore_()`, `saveHighScore_()`
 * - **FSM на основе таблицы переходов**: все состояния и события строго
 * типизированы.
 * - **C++17 возможности**:
 *   - `std::deque<SnakeSegment>` — эффективное добавление/удаление сегментов
 *   - `std::unordered_set<int>` — O(1) проверка занятых ячеек (ключ = y * cols
 * + x)
 *   - `thread_local std::mt19937` — потокобезопасный генератор для
 * `spawnApple_()`
 *   - `constexpr`, `noexcept`, `enum class` — современные и безопасные практики
 *
 * @note Все публичные методы — `noexcept`: исключения не покидают C++ слой,
 *       что предотвращает `std::terminate()` при вызове из C.
 * @note Класс **не предназначен для прямого использования** вне
 * `s21_snake.cpp`. Вся интеграция проходит через C API.
 * @note Потокобезопасность **не гарантируется** — все вызовы для одного
 * экземпляра должны выполняться из одного потока.
 * @note Файл компилируется как часть C++ модуля и линкуется с C-обёрткой.
 *
 * @warning Не изменяйте логику FSM без синхронизации таблицы `transitions_` и
 * `processEvent_()`.
 * @warning Не нарушайте инварианты `SnakeSegment`: (x, y) должны быть в
 * пределах поля.
 *
 * @author provemet
 * @version 1.0
...
const fsm_transition_t SnakeGame::transitions_[] = {
    ...
    // PAUSED → MOVE по событию PAUSE_TOGGLE
    {to_fsm_state(SnakeState::PAUSED), to_fsm_event(SnakeEvent::PAUSE_TOGGLE),
     to_fsm_state(SnakeState::MOVE), nullptr, &SnakeGame::on_state_enter_},

    // MOVE → GAME_OVER по событию TERMINATE
    ...
    // GAME_OVER → INIT по событию START (сброс состояния)
    {to_fsm_state(SnakeState::GAME_OVER), to_fsm_event(SnakeEvent::START),
 ...
 * ничего не делает.
 *
 * Выполняет полный цикл обновления состояния игры:
 * - обработка внутреннего таймера (генерация события TICK)
 * - перемещение змейки (если текущее состояние — MOVE)
 * - проверка столкновений с границами поля и собственным телом
 * - обработка съедания яблока (увеличение счёта, рост змейки)
 * - генерация нового яблока при необходимости
 * - обновление уровня и скорости (ускорение игры каждые 10 очков)
 * - управление переходами FSM в ответ на события
 * - обновление внутреннего представления игрового поля (info_.field)
 *
 * @note Функция должна вызываться регулярно (например, каждые 100–200 мс)
 *       для поддержания плавного игрового процесса.
 * @note Ничего не делает, если игра находится в состоянии PAUSED или GAME_OVER.
 * @note При достижении максимальной длины (SNAKE_MAX_LENGTH) игра завершается с
 * победой.
 * @note Безопасна для вызова с nullptr — игнорирует вызов.
 * @note Потоконебезопасна: все вызовы для одного экземпляра должны происходить
 * из одного потока.
 *
 * @warning Не вызывайте вручную события TICK — они обрабатываются
 * автоматически.
 * @warning Частый вызов приведёт к ускорению игры; редкий — к "лагам" и
 * пропуску шагов.
 * @warning Не вызывайте после destroy() — поведение не определено.
 *
 * @par Пример:
 * @code
 * while (!game_over) {
 *     snake_update(game);                    // Основной логический тик
 *     const GameInfo_t* info = snake_get_info(game);
 *     render_game(info);                     // Визуализация текущего состояния
 *     usleep(info->speed * 1000);            // Задержка в миллисекундах
 * }
 * @endcode
 */
...
/**
 * @private
 * @brief Приватный конструктор, инициализирующий новую игру
 *
 * Выполняет полную инициализацию внутреннего состояния экземпляра игры:
 * - настройку конечного автомата (FSM) в начальное состояние INIT
 * - выделение динамической памяти под игровое поле (info_.field) размером
 * SNAKE_FIELD_ROWS × SNAKE_FIELD_COLS
 * - инициализацию базовых параметров: счёт (0), уровень (1), скорость
 * (default_speed)
 * - загрузку сохранённого рекорда из файла (SNAKE_SCORE_FILE)
 * - создание директории сохранений, если она отсутствует (SNAKE_SCORE_DIR)
 * - инициализацию начального положения змейки (центр поля, направление вправо)
 * - генерацию первого яблока на свободной ячейке
 *
 * @note Конструктор вызывается **только** из статического метода create().
 *       Прямое создание объекта запрещено (см. delete-декларации
 * копирования/перемещения).
 * @note Все ресурсы, выделенные в конструкторе, освобождаются в деструкторе.
 * @note В случае неудачи при выделении памяти или доступе к файлу, объект всё
 * равно создаётся, но игра может работать с ограничениями (например, рекорд =
 * 0).
 *
 * @warning Не вызывайте этот конструктор напрямую — используйте только
 * snake_create() (C API).
 * @warning Не выбрасывает исключения — ошибка обрабатывается внутри (например,
 ...
 * или перезапуске после состояния GAME_OVER. Восстанавливает все параметры
 * в значения по умолчанию:
 * - сбрасывает счёт (score = 0), уровень (level = 1), скорость (speed =
 * default_speed)
 * - очищает тело змейки (body_.clear())
 * - размещает новую змейку заданной длины (SNAKE_INITIAL_LENGTH) в центре поля,
 * направление — вправо
 * - генерирует новое яблоко на свободной ячейке
 * - обновляет множество занятых ячеек (occupied_cells_)
 * - сбрасывает флаги состояния (info_.pause = false)
 *
 * @note Метод **не пересоздаёт** выделенную память под info_.field —
 * переиспользует существующие буферы.
 * @note Вызывается из обработчика события START в состояниях INIT и GAME_OVER.
 * @note После сброса состояние FSM переводится в MOVE (через переход таблицы).
 * @note Безопасен для вызова в любом состоянии.
 *
 * @warning Не вызывайте напрямую из C API — используйте
 * snake_handle_input(game, Start, false).
 * @warning Не меняет значение рекорда (high_score) — оно сохраняется между
 * играми.
 */
 ...
 * @param ctx Указатель на контекст FSM (в данном случае — экземпляр SnakeGame)
 *
 * Выполняет действия, необходимые при переходе в новое состояние:
 * - преобразует текущее состояние FSM в тип SnakeState
 * - обновляет флаги отображения (info_.pause)
 * - при необходимости запускает инициализацию новой игры
 *
 * Каждое состояние обрабатывается следующим образом:
 * - INIT:     сбрасывает игру через initialize_(), устанавливает pause = 0
 * - MOVE:     снимает паузу (pause = 0), продолжает игровой процесс
 * - PAUSED:   устанавливает флаг pause = 1 для отображения паузы
 * - GAME_OVER: снимает паузу (pause = 0), готовит интерфейс к перезапуску
 *
 * @note Является статическим колбэком для C-совместимого FSM — принимает void*
 * (fsm_context_t).
 * @note Автоматически вызывается при любом переходе состояния через FSM.
 * @note Не обрабатывает логику движения — только синхронизацию состояния и
 * UI-флагов.
 * @note Не выбрасывает исключения — noexcept (неявно, так как вызывается из C).
 *
 * @warning Не вызывайте напрямую — только через FSM.
 * @warning Не изменяйте ctx — он должен указывать на валидный экземпляр
 * SnakeGame.
 *
 * @par Пример поведения:
 * @code
 * // При переходе INIT → MOVE:
 * // - вызывается on_state_enter_
 * // - new_state == SnakeState::MOVE
 * // - info_.pause = 0 → отрисовка продолжается
 * @endcode
 */
void SnakeGame::on_state_enter_(fsm_context_t ctx) {
  ...
  switch (new_state) {
    case SnakeState::INIT:
      ...
      self->body_.clear();
      self->occupied_cells_.clear();
      self->direction_ = Direction::RIGHT;
      self->next_direction_ = Direction::RIGHT;
      self->info_.score = 0;
      self->apple_x_ = -1;
      self->apple_y_ = -1;
