 ...
 * - tetris_map_action_to_event() — сопоставление действий пользователя с
 * событиями FSM
 ...
 *
 * Архитектура следует паттерну "Фасад": скрывает сложность внутренней
 * реализации, обеспечивает чёткий, легко тестируемый и расширяемый интерфейс.
 *
 * @note Все функции являются потоконебезопасными и должны вызываться из одного
 * потока. Внутренняя структура TetrisGame скрыта через неполную декларацию.
 *
 * @warning Некорректные указатели (NULL) обрабатываются устойчиво — функции
 * безопасно завершаются без побочных эффектов. Не передавайте невалидные
 * указатели.
 *
 * @author provemet
 * @version 2.2
...
/**
 * @internal
 * @brief Преобразует пользовательское действие в событие конечного автомата
 * игры Тетрис
 *
 * Функция сопоставляет входное действие пользователя (нажатие клавиши,
 * удержание) с внутренним событием FSM (TetrisEvent), которое будет обработано
 * машиной состояний. Учитывает флаг удержания клавиши (hold) для различия между
 * мягким и жёстким падением.
 *
 * @param[in] action Тип действия пользователя, заданный через перечисление
 * UserAction_t. Возможные значения: Start, Left, Right, Down, Up, Action,
 * Pause, Terminate.
 * @param[in] hold   Флаг, указывающий, удерживается ли клавиша (true —
 * удержание, false — одиночное нажатие). Используется для определения поведения
 * при нажатии клавиши "вниз".
 *
 * @return Значение типа TetrisEvent, соответствующее семантике действия:
 *         - При action == Down и hold == true: TETRIS_EVT_DROP — мгновенное
 * падение (hard drop).
 *         - При action == Down и hold == false: TETRIS_EVT_MOVE_DOWN —
 * пошаговое движение вниз.
 *         - Остальные действия преобразуются в соответствующие события.
 *         - Неподдерживаемые или необрабатываемые действия (включая Up)
 * возвращают TETRIS_EVT_NONE.
 *
 * @details
 ...
 *
 * @note Функция является внутренней (static) и предназначена только для
 * использования в рамках модуля обработки ввода Тетриса. Не экспортируется за
 * пределы файла.
 *
 * @warning Событие TETRIS_EVT_NONE игнорируется конечным автоматом — такие
 * действия не приводят к изменению состояния игры.
 *
...
/**
 * @internal
 * @brief Обновляет структуру информации для отображения на основе текущего
 * состояния игры
 *
 * Функция синхронизирует содержимое внутреннего игрового поля и активной фигуры
 * с публичной структурой GameInfo_t, которая используется движком для
 * отрисовки. Копирует данные из внутреннего хранилища и накладывает текущую
 * падающую фигуру.
 *
 * @param[in] game Указатель на экземпляр игры Тетрис. Если равен NULL, функция
 * немедленно завершается.
 *
 * @details
 * Процесс обновления включает следующие шаги:
 * 1. Копирование зафиксированных блоков из game->field_storage в
 * game->info.field.
 * 2. Проверка состояния игры: если игра окончена или на паузе — активная фигура
 * не отображается.
 * 3. Проверка FSM-состояния: в состояниях инициализации и "game over" фигура не
 * рисуется.
 ...
 * - Значения в game->info.field устанавливаются в соответствии с цветами/типами
 * блоков (shape[i][j] — ненулевое значение интерпретируется как активный блок).
 * - Выход за пределы поля (например, при спавне) корректно обрабатывается.
 *
 * @warning Функция предполагает, что поля game->field_storage и
 * game->info.field уже выделены и имеют размер TETRIS_FIELD_ROWS ×
 * TETRIS_FIELD_COLS. Передача некорректного указателя может привести к
 * segmentation fault.
 *
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      if (shape[i][j]) {
        int y = cur->y + i;
        int x = cur->x + j;
        if (y >= 0 && y < TETRIS_FIELD_ROWS && x >= 0 &&
            x < TETRIS_FIELD_COLS) {
