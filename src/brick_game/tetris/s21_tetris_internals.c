 ...
 *
 * Задаёт начальное значение скорости (в условных единицах таймера),
 * используемое в начале игры. Скорость на последующих уровнях рассчитывается по
 * формуле: <code>current_speed = TETRIS_INITIAL_SPEED + level - 1</code>.
 *
 ...
 * - Фактически определяет частоту тиков `TETRIS_EVT_TICK`, управляющих
 * падением.
 ...
 * @code
 * #define TETRIS_TRANSITIONS_COUNT (sizeof(tetris_transitions) /
 * sizeof((tetris_transitions)[0]))
 * @endcode
 ...
 * - Является compile-time константой, пригодной для использования в статических
 * утверждениях.
 *
 * @warning Несанкционированное использование с другими данными приведёт к
 * логическим ошибкам. Не переиспользуйте этот макрос в других контекстах без
 * проверки типа.
 *
 ...
 *
 * Массив правил, определяющих поведение FSM: при каком текущем состоянии и
 * событии происходит переход в новое состояние, а также какие вызываются
 * колбэки.
 *
 ...
 * - SPAWN → GAME_OVER: при TETRIS_EVT_NONE (условный автопереход) — если новая
 * фигура не может быть размещена (поле переполнено).
 * - FALL → LOCK: при TETRIS_EVT_TICK — фиксация фигуры после неудачи в движении
 * вниз.
 * - FALL → FALL: при MOVE/ROTATE — события обрабатываются внутри состояния,
 * перехода нет.
 ...
 *
 * @note
 * - Таблица использует внешнюю структуру `fsm_transition_t` из общего
 * FSM-модуля.
 * - Колбэки (`on_enter_init`, `on_enter_spawn`, `on_enter_lock` и др.)
 * определены в том же модуле и управляют инициализацией, спавном, обновлением
 * скорости и т.д.
 * - FSM проверяет таблицу последовательно — первый подходящий переход
 * используется. Порядок элементов **имеет значение**.
 *
 * @warning
 * - Нарушение порядка (например, более общие правила перед частными) может
 *   привести к пропуску переходов.
 * - Изменение логики (например, добавление/удаление состояний или событий)
 * требует одновременного обновления TETRIS_TRANSITIONS_COUNT.
 * - Неправильные колбэки или состояния — источник зависаний, утечек или крахов.
 *
 * @see TetrisState, TetrisEvent, fsm_t, tetris_fsm_dispatch, fsm_transition_t,
 * TETRIS_TRANSITIONS_COUNT
 */
static const fsm_transition_t tetris_transitions[] = {
    /* INIT → SPAWN: старт игры, спавним первую фигуру */
    {TETRIS_STATE_INIT, TETRIS_EVT_START, TETRIS_STATE_SPAWN, NULL,
     on_enter_spawn},
    ...
    {TETRIS_STATE_SPAWN, TETRIS_EVT_NONE, TETRIS_STATE_GAME_OVER, NULL,
     on_enter_game_over},

    /* FALL → LOCK: фигура уперлась, фиксируем */
    {TETRIS_STATE_FALL, TETRIS_EVT_TICK, TETRIS_STATE_LOCK, NULL,
     on_enter_lock},

    /* FALL → LOCK: движение вниз и жёсткий дроп */
    {TETRIS_STATE_FALL, TETRIS_EVT_MOVE_DOWN, TETRIS_STATE_LOCK, NULL,
     on_enter_lock},
    {TETRIS_STATE_FALL, TETRIS_EVT_DROP, TETRIS_STATE_LOCK, NULL,
     on_enter_lock},
    ...
    /* Пауза */
    {TETRIS_STATE_FALL, TETRIS_EVT_PAUSE_TOGGLE, TETRIS_STATE_PAUSED, NULL,
     on_enter_paused},
    {TETRIS_STATE_PAUSED, TETRIS_EVT_PAUSE_TOGGLE, TETRIS_STATE_FALL,
     on_exit_paused, NULL},

    /* TERMINATE — в любой момент */
    {TETRIS_STATE_FALL, TETRIS_EVT_TERMINATE, TETRIS_STATE_GAME_OVER, NULL,
     on_enter_game_over},

    /* LOCK → SPAWN: после фиксации — новая фигура */
    {TETRIS_STATE_LOCK, TETRIS_EVT_TICK, TETRIS_STATE_SPAWN, NULL,
     on_enter_spawn},

    /* GAME_OVER → INIT: перезапуск */
    {TETRIS_STATE_GAME_OVER, TETRIS_EVT_NONE, TETRIS_STATE_INIT, NULL,
     on_enter_init},
};
 ...
 *
 * Создаёт матрицу размером rows × cols, где каждый элемент инициализирован
 * нулём. Алгоритм:
 ...
 *
 * @details
 * - Использует `calloc()` для строк и элементов — гарантируется обнуление
 * памяти.
 ...
 *
 * @note
 * - Корректно обрабатывает частичный отказ памяти (например, нехватку на 5-й
 * строке).
 * - Требуемая память: sizeof(int*) * rows + sizeof(int) * rows * cols.
 * - Результат должен быть освобождён через tetris_free_matrix() для избежания
 * утечек.
 *
 * @warning
 ...
 *
 * @see tetris_free_matrix(), tetris_game_create(), TETRIS_FIELD_ROWS,
 * TETRIS_FIELD_COLS
 */
 ...
 * Корректно обрабатывает NULL-указатель — в этом случае функция не выполняет
 * действий.
 *
 * @param[in] m     Указатель на матрицу (массив указателей на int). Может быть
 * NULL.
 * @param[in] rows  Количество строк в матрице. Должно соответствовать
 * количеству, переданному при выделении через tetris_alloc_matrix.
 *
 * @details
 ...
 * - Функция не устанавливает `m` в NULL — это остаётся ответственностью
 * вызывающего кода.
 *
 * @note
 * - Должна вызываться только для матриц, выделенных через
 * tetris_alloc_matrix().
 * - Используется в tetris_game_destroy() для очистки field_storage и
 * next_storage.
 * - Параметр `rows` должен быть точным — его несоответствие приведёт к
 * неопределённому поведению.
 *
 * @warning
 * - Повторный вызов с теми же параметрами (double free) вызывает неопределённое
 * поведение.
 * - Не является потокобезопасной — требует внешней синхронизации при
 * использовании из нескольких потоков.
...
TetrisGame *tetris_game_create(void) {
  ...
  game->field_storage =
      tetris_alloc_matrix(TETRIS_FIELD_ROWS, TETRIS_FIELD_COLS);
...
void tetris_game_reset(TetrisGame *game) {
  ...
  fsm_init(&game->fsm, (fsm_context_t)game, tetris_transitions,
           TETRIS_TRANSITIONS_COUNT, TETRIS_STATE_INIT);
}
/* -------------------------- Поле и превью ---------------------------- */
...
void tetris_update_next_preview(TetrisGame *game) {
  ...
  const int(*shape)[4] =
      tetris_getPieceData(game->next.type, game->next.rotation);
  if (!shape) return;
...
bool tetris_check_collision(const TetrisGame *game, const TetrisPiece *piece) {
  ...
  const int(*shape)[4] = tetris_getPieceData(piece->type, piece->rotation);
...
void tetris_lock_piece(TetrisGame *game) {
  ...
  const int(*shape)[4] =
      tetris_getPieceData(game->current.type, game->current.rotation);
  if (!shape) return;
  ...
  for (int row = 0; row < 4; ++row) {
    for (int col = 0; col < 4; ++col) {
      ...
      if (field_row < 0 || field_row >= TETRIS_FIELD_ROWS || field_col < 0 ||
          field_col >= TETRIS_FIELD_COLS) {
        continue;
      }
 ...
 *
 * Проверяет, что все ячейки в строке с индексом `row` содержат ненулевые
 * значения (то есть блоки), что означает её полное заполнение. Используется как
 * вспомогательная функция при анализе поля на предмет необходимости удаления
 * строк.
 *
 ...
 *
 * @note
 * - Является чистой (pure) функцией: зависит только от входных данных и не
 * изменяет состояние.
 * - Используется исключительно внутри tetris_clear_full_lines() для определения
 * строк к удалению.
 * - Линейная сложность O(cols) — оптимальна для однократной проверки строки.
 *
 * @warning
 * - Не выполняет отладочную печать — ошибка проявляется только через
 * возвращаемое значение.
 * - Не является потокобезопасной, если game->field_storage может изменяться из
 * другого потока.
 *
...
bool tetris_save_high_score(const TetrisGame *game) {
  ...
  if (mkdir_result != 0 && errno != EEXIST) {
    return false;  // Ошибка создания директории (и это не "директория уже
                   // существует")
  }
 ...
 *
 * Полностью сбрасывает состояние игры к начальному: обнуляет счёт, уровень,
 * флаги, игровое поле и превью. Восстанавливает корректные ссылки в GameInfo_t
 * и подготавливает экземпляр TetrisGame к началу новой партии.
 *
 * @param[in] ctx Указатель на контекст состояния — должен быть приведён к
 * TetrisGame*. Может быть NULL — в этом случае функция безопасно завершается.
 *
 * @details
 ...
 * - Синхронизация info.high_score с текущим значением game->high_score (из
 * памяти)
 * - Очистка игрового поля (tetris_clear_field) и буфера превью
 * (tetris_clear_next)
 *
 * @note
 * - Является внутренним колбэком FSM — **не предназначена для прямого вызова**.
 * - Вызывается при переходе в TETRIS_STATE_INIT как при старте игры, так и при
 * перезапуске.
 * - Гарантирует чистое состояние, но **не запускает логику игры** — спавн
 * фигуры происходит в TETRIS_STATE_SPAWN.
 *
 * @warning
 * - **Не загружает рекорд с диска** — ожидается, что game->high_score уже
 * установлен (например, через tetris_load_high_score() перед инициализацией
 * FSM).
 * - Не проверяет, инициализированы ли буферы field_storage/next_storage —
 * предполагается, что память выделена на более высоком уровне
 * (tetris_game_create).
 ...
 *
 * Инициирует появление новой активной фигуры: перемещает фигуру из `next` в
 * `current`, генерирует новую следующую фигуру, обновляет визуальное превью и
 * проверяет, может ли фигура быть размещена в начальной позиции. Если
 * размещение невозможно (например, из-за заполнения верхних строк),
 * устанавливается флаг `game_over`.
 *
 * @param[in] ctx Указатель на контекст состояния — должен быть приведён к
 * TetrisGame*. Может быть NULL — в этом случае функция безопасно завершается.
 *
 ...
 *    - Проверяет коллизию и при необходимости устанавливает game->game_over =
 * true.
 *
 * Функция не выполняет дополнительных проверок — полагается на логику
 * tetris_spawn_piece().
 *
 * @note
 * - Является внутренним колбэком FSM — **не предназначена для прямого вызова**.
 * - Вызывается как при старте игры (после TETRIS_STATE_INIT), так и после
 * фиксации фигуры.
 * - После успешного спавна игра ожидает переход в TETRIS_STATE_FALL для
 * управления падением.
 *
 * @warning
 * - **Не проверяет, была ли предыдущая фигура корректно зафиксирована** —
 *   это ответственность FSM и предыдущих состояний (например,
 * TETRIS_STATE_LOCK).
 * - Не очищает поле и не сбрасывает счёт — предполагается, что игра находится в
 * валидном состоянии.
 * - Не является потокобезопасной: требует эксклюзивный доступ к игровому
 * состоянию.
 *
 * @see tetris_fsm_dispatch(), tetris_spawn_piece(), tetris_state_init_enter(),
 * tetris_state_lock_enter()
 * @internal
 */
static void on_enter_spawn(fsm_context_t ctx) {
  TetrisGame *game = (TetrisGame *)ctx;
 ...
 * 3. Начисляет очки, обновляет уровень и скорость в зависимости от количества
 * линий.
 *
 ...
 *
 * Этот колбэк гарантирует, что все изменения состояния игры (счёт, поле,
 * уровень) происходят атомарно в один момент — при фиксации фигуры.
 *
 * @note
 * - Является внутренним колбэком FSM — **не предназначена для прямого вызова**.
 * - Вызывается автоматически, когда фигура не может двигаться вниз (после
 * TETRIS_STATE_FALL).
 * - После завершения FSM переходит в TETRIS_STATE_SPAWN для появления новой
 * фигуры.
 * - Даже если `tetris_lock_piece` не зафиксировал ни одной ячейки (например,
 * фигура частично вне поля), попытка удаления строк и начисления очков всё
 * равно выполняется.
 *
 * @warning
 * - Не проверяет корректность текущей фигуры — предполагается, что она валидна.
 * - Не изменяет `game->current` — очистка и смена фигуры происходят в
 * `tetris_state_spawn_enter`.
 * - Не является потокобезопасной: требует эксклюзивный доступ к игровому
 * состоянию.
 *
 * @see tetris_fsm_dispatch(), tetris_lock_piece(), tetris_clear_full_lines(),
 * tetris_apply_cleared_lines(), tetris_state_spawn_enter()
 * @internal
 */
static void on_enter_lock(fsm_context_t ctx) {
  TetrisGame *game = (TetrisGame *)ctx;
 ...
 *
 * Устанавливает флаг паузы в публичной структуре состояния игры (info.pause =
 * true), сигнализируя интерфейсу и внешним системам о том, что игра
 * приостановлена. Используется конечным автоматом при переходе в состояние
 * TETRIS_STATE_PAUSED.
 *
 * @param[in] ctx Указатель на контекст состояния — должен быть приведён к
 * TetrisGame*. Может быть NULL — в этом случае функция безопасно завершается.
 ...
 * - Устанавливает game->info.pause = true, чтобы UI мог отобразить состояние
 * паузы.
 * - Не изменяет игровое поле, текущую фигуру, счёт, уровень, скорость или флаги
 * игры.
 * - Все игровые данные остаются неизменными — игра может быть возобновлена в
 * любой момент.
 *
 * Функция входит в FSM-архитектуру и вызывается автоматически при событии
 * TETRIS_EVT_PAUSE_TOGGLE, когда текущее состояние — TETRIS_STATE_FALL.
 *
 * @note
 * - Является внутренним колбэком FSM — **не предназначена для прямого вызова**.
 * - Состояние паузы не влияет на логику таймера — ожидается, что внешняя
 * система (например, таймер в main loop) проверяет info.pause перед обработкой
 * тиков.
 * - После возобновления игра продолжается с той же фигуры и того же места.
 *
 * @warning
 * - Не проверяет корректность game->info — предполагается, что структура
 * инициализирована.
 * - Не является потокобезопасной: изменение info.pause должно быть
 * синхронизировано, если доступ к info осуществляется из другого потока.
 * - Не блокирует ввод — события могут продолжать поступать, но игнорируются в
 * состоянии PAUSED.
 *
 * @see tetris_fsm_dispatch(), tetris_state_paused_exit(), TetrisState,
 * fsm_process_event()
 * @internal
 */
static void on_enter_paused(fsm_context_t ctx) {
 ...
 * @details
 * - Приводит ctx к TetrisGame* и проверяет на NULL.
 * - Устанавливает game->info.pause = false, чтобы UI мог обновить отображение
 * состояния.
 * - Не изменяет игровое поле, текущую фигуру, счёт, уровень, скорость или флаги
 * игры.
 ...
 * из состояния TETRIS_STATE_PAUSED в TETRIS_STATE_FALL по событию
 * TETRIS_EVT_PAUSE_TOGGLE.
 *
 * @note
 * - Является внутренним колбэком FSM — **не предназначена для прямого вызова**.
 ...
 * - Не является потокобезопасной: изменение `info.pause` должно быть
 * синхронизировано, если доступ к `info` осуществляется из другого потока.
 * - Передача невалидного указателя (не NULL, но указывающего на мусор) приводит
 * к неопределённому поведению.
 *
 * @see tetris_fsm_dispatch(), tetris_state_paused_enter(), TetrisState,
 * fsm_process_event()
 * @internal
 */
static void on_exit_paused(fsm_context_t ctx) {
 ...
 *
 * Устанавливает флаг окончания игры в `true`, сигнализируя всем внешним
 * системам (например, пользовательскому интерфейсу), что текущая партия
 * завершена. Используется конечным автоматом при переходе в состояние
 ...
 * - Не изменяет игровое поле, счёт, уровень, фигуры или рекорд — все данные
 * остаются доступными.
 * - Не инициирует сохранение рекорда — ожидается, что
 * `tetris_save_high_score()` будет вызвана до или после этого перехода.
 *
 ...
 * - Не проверяет корректность `game->info` — предполагается, что структура
 * инициализирована.
 * - Не является потокобезопасной: изменение `game_over` должно быть
 * синхронизировано, если доступ к состоянию осуществляется из другого потока.
 * - Передача невалидного указателя (не NULL, но указывающего на мусор) приводит
 * к неопределённому поведению.
 *
 ...
 * мягкое (MOVE_DOWN) и жёсткое падение (DROP). Преобразует события для
 * корректной передачи в конечный автомат: подавляет обработанные действия и
 * возвращает специальные команды для переходов при блокировке падения.
 *
 * @param[in] game  Указатель на экземпляр TetrisGame. Может быть NULL — в этом
 * случае возвращается исходное событие без изменений.
 * @param[in] event Входящее событие (например, TETRIS_EVT_MOVE_LEFT,
 * TETRIS_EVT_TICK и др.)
 *
 * @return Преобразованное событие для FSM:
 *         - TETRIS_EVT_NONE — событие полностью обработано (например, движение
 * выполнено),
 *         - TETRIS_EVT_TICK — фигура не может двигаться вниз; требуется переход
 * в TETRIS_STATE_LOCK,
 *         - Исходное событие — если оно не обработано и должно быть обработано
 * FSM (например, пауза).
 *
 * @details
 * Логика обработки:
 * - TETRIS_EVT_MOVE_LEFT / RIGHT: вызывается tetris_move_piece(), при успехе
 * возвращается NONE.
 ...
 *   - Если движение невозможно — возвращается TETRIS_EVT_TICK (инициирует
 * LOCK).
 *   - Если возможно — возвращается NONE (состояние не меняется).
 * - TETRIS_EVT_DROP:
 *   - Выполняет hard drop: циклически вызывает tetris_move_piece(game, 0, 1),
 * пока возможно.
 *   - После остановки возвращает TETRIS_EVT_TICK для немедленной фиксации.
 * - Все остальные события (например, TETRIS_EVT_PAUSE_TOGGLE) возвращаются без
 * изменений, чтобы FSM мог корректно обработать переходы (например, в состояние
 * PAUSED).
 *
 * @note
 * - Функция вынесена для упрощения tetris_fsm_dispatch() — делает её чище и
 * понятнее.
 ...
 *
 * @warning
 * - Не проверяет текущее состояние — **должна вызываться только в
 * TETRIS_STATE_FALL**.
 * - Поведение не определено, если game->current содержит некорректные данные.
 * - Не является потокобезопасной: требует синхронизации при доступе к состоянию
 * игры.
 *
 * @see tetris_fsm_dispatch(), tetris_move_piece(), tetris_rotate_piece(),
 * TetrisEvent
 * @internal
 */
static TetrisEvent tetris_process_fall_input(TetrisGame *game,
                                             TetrisEvent event) {
