 ...
 * обработки ввода, игрового обновления и получения состояния. Использует принцип
 * инкапсуляции: внутренняя реализация скрыта за абстрактным типом void*.
 * Обеспечивает совместимость с общим движком игр через шаблон GameInterface_t.
 *
 ...
 *
 * Функция выделяет и инициализирует объект игры, включая игровое поле, статистику,
 * FSM (конечный автомат), текущую и следующую фигуры. Используется как точка входа
 * в публичный API игры. Возвращает абстрактный указатель для соблюдения принципа инкапсуляции.
 *
 * @return Указатель на вновь созданный экземпляр игры. Тип — void*, чтобы скрыть
 *         внутреннюю реализацию от внешнего кода. В случае ошибки выделения памяти
 *         или сбоя инициализации возвращается NULL.
 *
 * @details
 ...
 *
 * @see tetris_destroy(), tetris_game_create(), tetris_update(), tetris_get_info()
 */
 ...
 * выделенную память, уничтожает внутренние структуры данных, включая игровое поле,
 * фигуры, состояния FSM и сопутствующие буферы. После вызова указатель становится недействительным.
 *
 * @param[in] game Указатель на экземпляр игры, полученный через tetris_create().
 *                 Допускается передача NULL — в этом случае функция не выполняет никаких действий.
 *
 * @details
 ...
 *
 * @note Повторный вызов с уже освобождённым указателем приведёт к неопределённому поведению.
 *       Убедитесь, что после вызова tetris_destroy() указатель больше нигде не используется.
 *
 * @warning Не используйте данный указатель после вызова функции. Повторное освобождение
 *          одной и той же памяти (double free) вызовет критический сбой программы.
 *
 ...
 *
 * Преобразует действия пользователя (нажатие клавиш) в события конечного автомата (FSM),
 * которые затем обрабатываются в соответствии с текущим состоянием игры. Поддерживает
 * как однократные нажатия, так и удержание клавиш (через флаг hold).
 *
 * @param[in] game    Указатель на экземпляр игры, созданный через tetris_create().
 *                    Если равен NULL, функция безопасно завершается без выполнения действий.
 * @param[in] action  Тип действия пользователя, заданный перечислением UserAction_t.
 *                    Допустимые значения:
 *                    - Start:      начать игру или перезапустить после окончания
 ...
 *                    - Up:         нет действия (заглушка), оставлено для совместимости
 *                    - Down:       ускорить падение фигуры (вниз)
 *                    - Action:     повернуть фигуру по часовой стрелке
 * @param[in] hold    Флаг удержания клавиши:
 *                    - true:  действие вызвано длительным нажатием (например, ускоренное падение)
 ...
 *    функции tetris_map_action_to_event(), которая учитывает тип действия и флаг hold.
 * 3. Если событие определено (не TETRIS_EVT_NONE), оно отправляется в FSM через tetris_fsm_dispatch().
 *
 * Примеры:
 ...
 * @note Функция потоконебезопасна — должна вызываться из того же потока, что и tetris_update().
 *
 * @warning Не передавайте невалидные значения action. Поведение в таких случаях не определено.
 *          После вызова tetris_destroy() использование указателя game приведёт к аварийному завершению.
 *
 ...
 *
 * Функция вызывается регулярно (по таймеру) для продвижения логики игры во времени.
 * Преобразует временной тик в событие конечного автомата (FSM) — TETRIS_EVT_TICK,
 * которое обрабатывается в соответствии с текущим состоянием игры.
 *
 * @param[in] game Указатель на экземпляр игры, созданный через tetris_create().
 *              Может быть NULL — в этом случае функция безопасно завершается без действий.
 *
 * @details
 ...
 *
 * @note Вызов должен происходить с фиксированной частотой (например, раз в 50–500 мс),
 *       чтобы обеспечить плавность игрового процесса. Частота зависит от текущего уровня.
 *
 * @warning Функция не является потокобезопасной. Все вызовы игровых функций
 *          должны происходить из одного потока. Не передавайте невалидный указатель.
 *
 * @see tetris_create(), tetris_handle_input(), tetris_fsm_dispatch(), TETRIS_EVT_TICK
 */
 ...
 *
 * @param[in] game Указатель на экземпляр игры, полученный через tetris_create().
 *                 Если передан NULL, функция возвращает NULL.
 *
 * @return Указатель на константную структуру GameInfo_t, содержащую данные для отрисовки,
 *         или NULL, если указатель на игру недействителен.
 *
 * @details
 ...
 * - Накладывает поверх активную падающую фигуру (если игра не на паузе и не завершена)
 ...
 *
 * @note Указатель остаётся валидным только до следующего вызова tetris_update(),
 *       tetris_handle_input() или tetris_destroy(). Не сохраняйте его надолго.
 *       Данные не копируются — возвращается ссылка на внутреннее состояние.
 *
 ...
 *
 * @see GameInfo_t, tetris_update_info_view(), tetris_update(), tetris_handle_input()
 */
const GameInfo_t *tetris_get_info(const void *game);

/**
 * @brief Возвращает интерфейс взаимодействия с игрой "Тетрис" для интеграции в общий игровой движок
 *
 * Функция формирует и возвращает структуру GameInterface_t, содержащую функции управления
 * игрой Тетрис. Используется движком игр (brick_game) для динамической регистрации
 * и управления играми через единый API.
 *
 * @param[in] id Идентификатор запрашиваемой игры. Должен соответствовать GAME_TETRIS
 *               для получения валидного интерфейса.
 *
 * @return Структура GameInterface_t, заполненная указателями на функции игры Тетрис,
 *         если id == GAME_TETRIS. В противном случае возвращается пустая структура (все поля — 0).
 *
 * @details
 ...
 * - get_info:  указатель на tetris_get_info() — получение состояния для отрисовки
 *
 * @note Функция потокобезопасна, если не используется совместно с изменением GAME_TETRIS.
 *       Предназначена для вызова при инициализации игрового движка.
 *
 ...
 *
 * @see GameInterface_t, GameId_t, tetris_create(), tetris_handle_input(), s21_bgame.h
 */
