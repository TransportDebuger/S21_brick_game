...
/**
 * @def BGAME_SCORE_DIR
 * @brief Имя скрытой директории для хранения данных игры.
 *
 * Директория используется для сохранения рекордов, настроек или других
 * пользовательских данных между сессиями игры. Имя начинается с точки,
 * что делает её скрытой в большинстве файловых менеджеров (например, в Linux и macOS).
 *
 * Пример пути: ~/.brickgame
 */
#define BGAME_SCORE_DIR ".brickgame"
...
/**
 * @brief Загружает игру как динамический плагин.
 *
 * Эта функция загружает динамическую библиотеку (.so/.dll) по указанному пути,
 * ищет в ней функцию с именем "get_game_interface", вызывает её и регистрирует
 * возвращённый интерфейс игры в глобальном реестре.
 *
 * @param[in] path Абсолютный или относительный путь к файлу плагина.
 * @return 0 при успехе, отрицательное значение при ошибке.
 *         -1: Файл не найден или не доступен.
 *         -2: Не удалось загрузить динамическую библиотеку.
 *         -3: Символ "get_game_interface" не найден в библиотеке.
 *         -4: Игра с таким же GameId уже зарегистрирована.
 */
int bg_load_plugin(const char *path);

/**
 * @brief Выгружает все динамически загруженные плагины.
 *
 * Перебирает реестр игр и выгружает все библиотеки, которые были загружены
 * динамически. Уничтожает все активные экземпляры таких игр. Плагины,
 * статически скомпонованные в основную библиотеку, не затрагиваются.
 *
 * @return 0 при успехе, отрицательное значение при ошибке выгрузки.
 */
int bg_unload_all_plugins(void);

/**
 * @brief Возвращает количество активно зарегистрированных игр в реестре.
 *
 * Учитывает как встроенные, так и динамически загруженные игры.
 *
 * @return Количество зарегистрированных игр.
 */
int bg_get_registered_count(void);

/**
 * @brief Заполняет буфер идентификаторами всех зарегистрированных игр.
 *
 * Копирует идентификаторы всех игр из реестра в предоставленный буфер.
 * Если буфер слишком мал, копируется только первые `buffer_size` идентификаторов.
 *
 * @param[out] buffer Указатель на массив, куда будут скопированы идентификаторы.
 * @param[in] buffer_size Размер массива `buffer`.
 * @return Количество идентификаторов, которые были скопированы в буфер.
 */
